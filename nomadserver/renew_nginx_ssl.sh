...
docker run -it -v "/etc/letsencrypt:/etc/letsencrypt" --rm --entrypoint "" certbot/certbot sh -c "certbot renew"

# list the docker containers |
# find the nginx one |
# execute `nginx -s reload` on that container to reload the certs 
docker ps --format '{{.Names}}' | grep "^nginx-" | xargs -I {} docker exec -t {} "/usr/sbin/nginx" "-s" "reload"
